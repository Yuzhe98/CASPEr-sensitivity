...
# This script is mostly generated by ChatGPT. 
##########################################
# TODO check if all values and units are correct. 
# TODO test
...
ureg.define("parsec = 3.085677581e16 * meter = pc")  # Parsec
ureg.define("light_year = 9.4607e15 * meter = ly")   # Light-Year
ureg.define("astronomical_unit = 1.495978707e11 * meter = au")  # Astronomical Unit
ureg.define("solar_mass = 1.98847e30 * kilogram = M_sun")       # Solar Mass
ureg.define("light_speed = 299792458.0 * m / s = c")       # Solar Mass
ureg.define("Phi_0 = 4.135667696e-15 / 2 * Wb")       # Flux quanta
# ureg.define("nanohenry = 1e6 * H = nH = nanoHenry")       # 
ureg.define("ppm = 1e-6")       # parts per million
...
class PhysicalQuantity:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, PhysicalQuantity):
            ...
            result = PhysicalQuantity(other * self.quantity.magnitude, str(self.quantity.units)).quantity
        else:
    ...
    def __rmul__(self, other):
        ...
        if isinstance(other, PhysicalQuantity):
            ...
            result = PhysicalQuantity(self.quantity.magnitude * other, str(self.quantity.units)).quantity
        else:
            TypeError(f"Unsupported division with type {type(other)}")
        return PhysicalQuantity(result.magnitude, str(result.units))
    ...
    def __rtruediv__(self, other):
        if isinstance(other, PhysicalQuantity):
            ...
            result = PhysicalQuantity(other / self.quantity.magnitude, "(" + str(self.quantity.units) + ")**-1").quantity
        else:
    ...
    def convert_to(self, unit):
        ...
        return PhysicalQuantity(converted_quantity.magnitude, str(converted_quantity.units))
    ...
    def example(self):
        ...
        try:
            ...
            print(e)  # Output: Cannot convert from 'electron_volt' ([mass]*[length]**2/[time]**2) to 'meter' ([length])


if __name__ == "__main__":
    pass
    # example = PhysicalQuantity(2, 'm')
    # example.example()
...
# Avogadro's number
NA = PhysicalQuantity(6e23, "mol**(-1)")
...
gamma_p = PhysicalQuantity(2 * pi * 42.5774825e6, "hertz / tesla")
